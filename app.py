...
tab_main, tab_help, tab_inplace, tab_yearly = st.tabs(
    [
        "Generate Schedules",
        "Help & Guide",
        "How to Download InPlace",
        "Yearly Form Setup",
    ]
)
...
with tab_help:
    ...
    with st.container(border=True):
        ...
        steps = [
            (
                "1",
                "Upload both files",
                "In the Generate Schedules tab, upload the InPlace file on the left and the Google Form CSV on the right.",
            ),
            (
                "2",
                "Select the year",
                "A year dropdown appears automatically based on the InPlace data. Choose the correct placement year.",
            ),
            (
                "3",
                "Check the summary",
                "Confirm the detected placement period, student count, and school count look correct.",
            ),
            (
                "4",
                "Choose schools",
                "Select All schools to export everything, or use Selected schools only to test with one school first.",
            ),
            (
                "5",
                "Generate",
                "Click the Generate button and wait for the progress bar to complete. Do not close the browser.",
            ),
            (
                "6",
                "Download",
                "Click the Download button to save your files. Multiple schools are bundled into a ZIP file.",
            ),
        ]
    ...
    try:
        ...
        st.info(
            "Place the file 'Placement_Schedule_Generator_User_Guide.docx' in the same folder as app.py to enable this download."
        )
...
with tab_inplace:
    ...
    with st.container(border=True):
        ...
        <p style="font-size:0.78rem;color:{MONASH_GREY};margin-top:10px;">
          Note: Student ID is no longer used — you do not need to add a Student Code column.
        </p>
        """,
            unsafe_allow_html=True,
        )
    ...
    with st.container(border=True):
        ...
        steps_ip = [
            (
                "1",
                "Log in to InPlace",
                "Open your browser and go to the InPlace website. Sign in with your Monash staff credentials.",
            ),
            (
                "2",
                "Go to iReport",
                "From the InPlace menu, navigate to iReport. [Add your team's exact navigation path here.]",
            ),
            (
                "3",
                "Select the correct report",
                "Choose the placement report for 1st and 2nd year students, non-method and non-HPE, for the current year.",
            ),
            (
                "4",
                "Download as Excel",
                "Click the download or export button. Choose Excel (.xlsx). Save it to your Desktop or Downloads folder.",
            ),
            (
                "5",
                "Do not edit the file",
                "Upload it to the app exactly as downloaded. Do not open it in Excel or rename any columns.",
            ),
        ]
        html = ""
...
with tab_yearly:
    st.markdown(
        f"""
    <div style="padding:0.5rem 0 0.2rem">
    <h3 style="color:#6A1B9A;font-size:1.1rem;font-weight:700;margin-bottom:4px;">Setting Up the Form for a New Year</h3>
    <p style="color:{MONASH_GREY};font-size:0.85rem;margin-bottom:1.2rem;">
        Each year the form needs new placement dates. The easiest and safest way is to copy last year's form and change only the dates.
    </p>
    </div>
    """,
        unsafe_allow_html=True,
    ...
    with st.container(border=True):
        st.markdown(
            f"""
        <div class="step-header">
          <span class="step-badge" style="background:#F57C00;">The golden rule</span>
          <p class="step-title">Only the dates change each year</p>
        </div>
        <div style="background:#FFF3E0;border:2px solid #F57C00;border-radius:8px;
                    padding:14px 18px;">
          <p style="font-size:0.88rem;color:{MONASH_DARK};margin:0;line-height:1.7;">
            Only <strong>one</strong> thing changes each year: the list of placement days.
            Everything else — the questions, the column names, the structure — must stay
            <strong>exactly the same</strong>, or the app will not be able to read the responses.
          </p>
        </div>
        """,
            unsafe_allow_html=True,
    ...
    with st.container(border=True):
        st.markdown(
            f"""
        <div class="step-header">
          <span class="step-badge" style="background:#6A1B9A;">Where the files live</span>
          <p class="step-title">Shared Google Drive folder</p>
        </div>
        <p style="font-size:0.85rem;color:{MONASH_DARK};margin-bottom:10px;">
          The form and its response sheet are stored in a shared Drive folder. Always save new
          versions here so the whole team can find them.
        </p>
        <a href="https://drive.google.com/drive/folders/1x4hV2psq6Bt4mvnZkyFJ5w8kS_6ZSX_H?usp=drive_link"
           target="_blank"
    ...
    with st.container(border=True):
        ...
        <div class="step-header">
          ...
          <p class="step-title">How to create the new year's form</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        steps_yearly = [
            (
                "1",
                "Open last year's form",
                "Go to the shared Drive folder above and open the most recent placement form.",
            ),
            (
                "2",
                "Make a copy",
                "Click the three-dots menu (⋮) in the top-right of Google Forms, then choose 'Make a copy'.",
            ),
            (
                "3",
                "Rename with the new year",
                "Name the copy clearly with the year, e.g. 'Placement Availability Form 2027'.",
            ),
            (
                "4",
                "Save it in the shared Drive folder",
                "Set the copy's location to the shared Drive folder so the whole team can access it.",
            ),
            (
                "5",
                "Open the days question",
                "In the new copy, scroll to 'What days will you be attending placement?' and click to edit it.",
            ),
            (
                "6",
                "Delete all old dates",
                "Remove every date option from last year. Do not leave any old dates behind.",
            ),
            (
                "7",
                "Add the new dates",
                "Type each new day as its own option using the exact format below. Add all working days across the 3-week period.",
            ),
            (
                "8",
                "Save and test",
                "Submit a test response to confirm the dates appear correctly and a row is added to the response sheet.",
            ),
        ]
        html = ""
        for num, title, detail in steps_yearly:
            html += f"""
            <div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:14px;">
        ...
        <div style="background:#F3E5F5;border-left:4px solid #6A1B9A;border-radius:4px;
                    padding:10px 14px;font-size:0.83rem;color:{MONASH_DARK};margin-top:6px;">
          <strong style="color:#6A1B9A;">Date format — must be exact:</strong><br>
          Write each date as &nbsp;<strong>Weekday &nbsp; Month &nbsp; Day</strong><br>
          <span style="color:#6A1B9A;font-weight:600;">Monday July 19 &nbsp;&nbsp; Tuesday July 20 &nbsp;&nbsp; Friday August 6</span><br>
          <span style="color:{MONASH_GREY};font-size:0.8rem;">No year. No commas. No slashes. No ordinals (not 19th, not 20th).</span>
        </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    with st.container(border=True):
        st.markdown(
            f"""
        <div class="step-header">
          <span class="step-badge" style="background:#F57C00;">Do not change</span>
          <p class="step-title">Everything except the dates stays the same</p>
        </div>
        <ul style="font-size:0.83rem;color:{MONASH_DARK};margin:0;padding-left:20px;line-height:1.9;">
          <li>Do not rename, delete, reorder, or add any other questions</li>
          <li>The app matches columns by name, so the question wording must stay the same</li>
          <li>Changing a question's wording changes its column name — the app may then fail to find it</li>
        </ul>
        <p style="margin-top:10px;font-size:0.82rem;color:{MONASH_GREY};">
          Because you copied the form, all the correct columns (Timestamp, Email address, Full Name,
          the days question, and the confirmation question) are already in place — just leave them as they are.
        </p>
        """,
            unsafe_allow_html=True,
        )
...
with st.container(border=True):
    ...
    if inplace_file and form_file:
        ...
        selected_year = st.selectbox(
            "Year", options=available_years, label_visibility="collapsed"
        )

        # Filter InPlace by year and other rules
        inplace_df, window_start, window_end = filter_inplace(
            inplace_raw, selected_year
        )
        ...
        try:
            ...
            try:
                ...
                form_years = sorted(
                    _pd.to_datetime(
                        form_df["timestamp"], dayfirst=True, errors="coerce"
                    )
                    .dt.year.dropna()
                    .astype(int)
                    .unique()
        ...
        short_labels, week_of, week_labels, range_label = build_schedule_labels(
            ordered_dates
        )
        ...
        no_placement_df = build_no_placement_df(
            no_placement_inplace, form_df, day_lookup
        )
        ...
        <div class="metrics-row">
          ...
          <div class="metric-card" style="border-left-color:#7B1FA2;">
            <div class="metric-label">EDU (no placement needed)</div>
            <div class="metric-value" style="color:#7B1FA2;">{status_counts["edu"]}</div>
            <div class="metric-sub">excluded from export</div>
          </div>
...
with st.container(border=True):
    ...
    if generate:
        schools_data = []
        total_files = n_schools + (
            1 if include_no_placement and len(no_placement_df) > 0 else 0
        )
        ...
        if include_no_placement and len(no_placement_df) > 0:
            ...
            status_text.markdown(
                f'<div class="progress-label">Building: No Placement file...</div>',
                unsafe_allow_html=True,
            )
            ...
            schools_data.append(
                (f"No_Placement_{selected_year}.xlsx", workbook_to_bytes(wb_np))
            )
